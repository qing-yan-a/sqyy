...
请确认创建完成后回复"已创建"。"""
    ...
    def _scheduled_sync(self) -> dict:
        ...
        # 发送近24小时session信息给云端
        self.logger.info("发送近24小时session信息给云端...")
        self._send_session_info_to_cloud()
    ...
    def _send_session_info_to_cloud(self) -> bool:
        """发送近24小时session信息给云端
        
        Returns:
            bool: 是否发送成功
        """
        self.logger.info("开始发送session信息给云端...")
        
        # 获取session相关配置
        session_hours = self.config["sync"].get("session_hours", 24)
        
        # 生成session摘要文件
        summary_path = self.file_manager.generate_session_summary(hours=session_hours)
        
        if not summary_path:
            self.logger.info("没有需要发送的session信息")
            return True
        
        # 上传session_summary.txt
        upload_result = self.file_manager.upload_file(summary_path)
        
        if not upload_result["success"]:
            self.logger.error("上传session摘要文件失败")
            return False
        
        download_url = upload_result["download_url"]
        if not download_url:
            self.logger.warning("没有获取到session摘要文件下载链接")
            return False
        
        # 发送消息给云端
        try:
            message = f"""你目前是在云端的服务器上，你刚才下载的文件是本地小宋的信息，如果有不一致就按照你的实际情况修改。不要把刚才下载的那几个文件备份到tongbu文件夹里，另外，这里有我们近期的session信息：
{download_url}"""
            
            reply = self.send(message, timeout=120)
            self.logger.info(f"session信息发送成功，云端回复: {reply[:200]}...")
            ...
            self.sync_history.add_record(
                ...
                status="success",
                message=f"session信息发送成功",
            )
            
            return True
            
        except Exception as e:
            self.logger.error(f"发送session信息失败: {e}")
            self.sync_history.add_record(
                action="session_sync",
                file_path=summary_path,
                status="failed",
                message=f"发送session信息失败: {e}",
            )
            return False
